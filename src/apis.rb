...
module AIOffice_tip
  ...
  class Sample < KSO_SDK::JsBridge
    ...
    def w_typeText(text)
      KSO_SDK.getApplication().Selection.TypeText(text)
    end
    ...
    def p_insertTextToSelectedShape(text)
      KSO_SDK.getApplication().ActiveWindow.Selection.ShapeRange(1).TextFrame.TextRange.InsertAfter(text)
    end
    ...
    def s_getActiveCell()
      KSO_SDK::Application.ActiveCell.Address
    end
	...
	# common
    def c_getAppType()
	  KSO_SDK::getAppType().to_s
    end
	
    def c_wirteNewFile(filePath, text)
	  file = File.new(filePath, "w")
	  file.puts text
	  file.close
    end
	
    def c_readFile(filePath)
      if File.exist?(filePath)
